...
-- Check for file changes made outside nvim to prevent stale LSP diagnostic errors
...
-- Show a small floating window when pausing on a line with diagnostics
vim.api.nvim_create_autocmd('CursorHold', {
  callback = function()
    vim.diagnostic.open_float(nil, {
      focus = false, -- don’t steal the cursor
      border = 'rounded', -- looks nicer
      source = 'if_many', -- show the source only if >1 LSPs
      scope = 'cursor', -- just the diagnostics under cursor
    })
  end,
})
...
require('lazy').setup({
  ...
  },
  {
    ...
    config = function()
      ...
      -- Start with Neovim's default capabilities, then extend
      local capabilities = vim.lsp.protocol.make_client_capabilities()
      capabilities = vim.tbl_deep_extend('force', capabilities, require('blink.cmp').get_lsp_capabilities())

      -- Enable LSP file watching for external changes
